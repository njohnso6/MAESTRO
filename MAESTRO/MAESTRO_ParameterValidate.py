...
def scrna_validator(args):
    ...
    if args.platform == "Dropseq":
        if args.input_path == "":
            logging.error("--input_path is required for Dropseq data. Please specify the directory where fastq files are stored!")
            exit(1)
        if args.fastq_barcode == "":
            logging.error("--fastq-barcode is required for Dropseq data. Please specify the barcode fastq file!")
            exit(1)
        if args.fastq_transcript == "":
            logging.error("--fastq-transcript is required for Dropseq data. Please specify the transcript fastq file!")
            exit(1)
        if args.whitelist == "":
            logging.error("--whitelist is required for Dropseq data. Please provide the barcode whitelist.")
            exit(1)
    ...
    if os.path.exists(args.lisadir):
        pass
    else:
        logging.error("Please specify the signature built in MAESTRO or provide customized signature file. See --signature help for more details!")
        exit(1)
