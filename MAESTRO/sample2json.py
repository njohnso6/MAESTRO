...
def sample_parser(subparsers):
    ...
    group_input.add_argument("--assay_type", dest = "assay_type", choices = ["scrna", "scatac", "multiome"],
    ...
    group_input.add_argument("--data_dir", dest = "data_dir",
        help="Required for single modality assay. the FULL path to the fastq folder or the fragment folder.")
    group_input.add_argument("--scatac_dir", dest = "scatac_dir",
        help="Required for mutiome asssay. The FULL path to the scatac-seq fastq folder.")
    group_input.add_argument("--scrna_dir", dest = "scrna_dir",
        help="Required for mutiome asssay. The FULL path to the scrna-seq fastq folder.")

def sample_json(args):
    """
    Generate samples.json file.
    """
    assert args.assay_type == "scrna" or args.assay_type == "scatac" or args.assay_type == "multiome", "please specify assay_type."
    ...
    if not args.assay_type == "multiome":
        assert args.data_dir is not None, "please provide the path to the fastq or fragment folder."
    if args.assay_type == "multiome":
        assert args.scatac_dir is not None, "please provide the path to the scatac-seq fastq folder."
        assert args.scatac_dir is not None, "please provide the path to the scrna-seq fastq folder."
    ...
    if args.assay_type == "multiome":
            if not args.data_type == "fastq":
                print("for multiome assay type, only fastq data type is supported.")
                sys.exit(1)
            if not args.platform == "10x-genomics":
                print("We only support data from 10X genomics for multiome assay type.")
            FILES = defaultdict(lambda: defaultdict(list))
            for root, dirs, files in os.walk(args.scrna_dir):
                for file in files:
                    full_path = join(root, file)
                    if file.endswith("fastq.gz"):
                        m = re.search(r"([A-Z0-9a-z_]+)_S[0-9]+_(L[0-9]{3})_([IR][12])_[0-9]+.fastq.gz", file)
                        ...
                        if m:
                            ...
                            reads = m.group(3)
                            FILES[sample][reads].append(full_path)
            for root, dirs, files in os.walk(args.scatac_dir):
                for file in files:
                    full_path = join(root, file)
                    if file.endswith("fastq.gz"):
                        m = re.search(r"([A-Z0-9a-z_]+)_S[0-9]+_(L[0-9]{3})_([IR][123])_[0-9]+.fastq.gz", file)
